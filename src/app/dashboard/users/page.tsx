...
const UsersPage = async ({ searchParams }: { searchParams: Promise<{ page?: string, role?: string, team?: string }> }) => {
  ...
  // --- THE NEW BRAINS: 3-STATE BUDGET LOGIC ---
  ...
  allTeams?.forEach(team => {
    ...
    // Save to the map for UI Badges in the table
    teamStatusMap[team.id] = status;

    // FIX: Push to assignment array if the team is ACTIVE *OR* UPCOMING!
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 flex-1 min-h-0">
        ...
        <div className="xl:col-span-1 xl:h-full flex flex-col min-h-0">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 xl:overflow-y-auto max-h-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        {/* RIGHT COLUMN: The Directory Table */}
        <div className="xl:col-span-2 xl:h-full flex flex-col min-h-0">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-4 xl:mb-6 flex-shrink-0">
            <div>
              ...
              <span className="text-sm font-medium text-slate-500 inline-block mt-1">
                Found {count || 0} Members
              </span>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex-1 flex flex-col min-h-0">
                        ...
                        <td className="px-6 py-4">
                          {teamName !== 'Unassigned' ? (
                            <div className="flex flex-col items-start gap-1">
                              ...
                              {/* NEW DYNAMIC BADGES BASED ON 3-STATE LOGIC */}
                              {status === 'EXPIRED' && (
                                <span className="text-[9px] font-bold text-red-600 bg-red-50 border border-red-100 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                  Expired Budget
                                </span>
                              )}
                              {status === 'UPCOMING' && (
                              ...
                              {status === 'NO_BUDGET' && (
                                <span className="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                  Needs Budget
                                </span>
                              )}
                            </div>
            ...
            <div className="border-t border-slate-200 bg-white px-6 py-4 flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex gap-2 w-full sm:w-auto justify-between sm:justify-end">
                {currentPage > 1 ? (
                  <Link 
                    href={buildPaginationLink(currentPage - 1)} 
                    className="px-4 py-2 border border-slate-200 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50 transition"
                  >
                    Previous
                  </Link>
                ) : (
                  <button disabled className="px-4 py-2 border border-slate-200 rounded-md text-sm font-medium text-slate-400 bg-slate-50 cursor-not-allowed">
                    Previous
                  </button>
                )}
